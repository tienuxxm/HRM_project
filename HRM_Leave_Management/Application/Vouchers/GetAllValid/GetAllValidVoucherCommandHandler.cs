...
public class GetAllValidVoucherCommandHandler : ICommandHandler<GetAllValidVoucherCommand, List<VoucherResponse>>
{
    ...
    public GetAllValidVoucherCommandHandler(IVoucherRepository voucherRepository, IMemberContext memberContext,
    ...
    public async Task<Result<List<VoucherResponse>>> Handle(GetAllValidVoucherCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var query = _voucherRepository.GetEntitiesAsQueryable()
            ...
            .Where(v => memberVouchers.All(mv => !mv.Equals(v.Id)) &&
                        ...
                        (!v.IsVoucherDefault.HasValue || !v.IsVoucherDefault.Value) && v.VoucherDefaultType == null)
            .AsQueryable();
        ...
        var vouchers = (await query.ToListAsync(cancellationToken))
            ...
            .Select(x => new VoucherResponse
            {
                ...
                QrCodeImageUrl = x.QrCodeImageUrl != null
                    ? _awsS3Service.GetUrlPresign(x.QrCodeImageUrl.Value)
                    : string.Empty,
                Place = x.Place?.Value,
                Status = x.Status,
