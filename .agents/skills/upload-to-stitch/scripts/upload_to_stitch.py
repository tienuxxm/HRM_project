...
def parse_args():
  ...
  parser.add_argument(
      "--api-key",
      default=None,
      help="API key for the Stitch API. If not provided, resolves from mcp_config.json.",
  )
  parser.add_argument(
      "--title",
      default=None,
  ...
  parser.add_argument(
      "--device-type",
      default=None,
      help="Optional device type (e.g. MOBILE, DESKTOP)",
  )
  parser.add_argument(
      "--width",
      type=int,
      default=None,
      help="Optional screen width",
  )
  parser.add_argument(
      "--height",
      type=int,
      default=None,
      help="Optional screen height",
  )
    ...
    if config_path.exists():
      try:
        with open(config_path, "r", encoding="utf-8") as f:
          config = json.load(f)
          stitch_args = config.get("mcpServers", {}).get("StitchMCP", {}).get("args", [])
          for i, arg in enumerate(stitch_args):
            if arg == "--header" and i + 1 < len(stitch_args):
              header_val = stitch_args[i + 1]
              if header_val.startswith("X-Goog-Api-Key:"):
                api_key = header_val.split("X-Goog-Api-Key:", 1)[1].strip()
                break
      except Exception as e:
        print(f"Warning: Failed to parse mcp_config.json: {e}")
    if not api_key:
      import os
      api_key = os.environ.get("STITCH_API_KEY")
    if not api_key:
      print("Error: --api-key is required or must be loadable from mcp_config.json or STITCH_API_KEY env var.")
      sys.exit(1)
